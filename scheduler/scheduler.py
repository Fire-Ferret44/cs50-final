"""Basic CSP scheduler for internal medicine doctors using backtracking search"""
...
def generate_schedule(
    ...
    int_doctors = load_doctors(
        ...
        preferences_path=None, # preferences not used in this version
    ...
    #2.2 check if day is counted as a weekend shift
    def is_weekend_shift(shift):
        day_info = shift_calendar.calendar[shift.date]
        return day_info["day_type"] in ("friday", "saturday", "sunday")
    ...
    doc_hours = {doc: 0 for doc in doctors} #to track number of hours per doctor
    doc_weekend_hours = {doc: 0 for doc in doctors} #to track number of weekend hours per doc
    ...
    def order_domain_values(shift_id, domains, doc_hours, doc_weekend_hours, shift_by_id):
        ...
        shift = shift_by_id[shift_id]
        is_weekend = is_weekend_shift(shift) #this would include Friday-Sunday and PH
    ...
    def violates_consecutive_days(shift_id, doc, assignment, shift_by_id):
        ...
        for assigned_shift_id, assigned_doc in assignment.items():
            ...
            assigned_date = shift_by_id[assigned_shift_id].date
            if abs((assigned_date - shift_date).days) == 1: #abs value of timedelta 1 is consec.
                return True
    ...
    def backtrack(assignment, domains):
        ...
        for doc in order_domain_values(
            ...
            if is_consistent(shift_id, doc, assignment, shift_by_id, int_doctors):
                ...
                if is_weekend_shift(shift_by_id[shift_id]):
